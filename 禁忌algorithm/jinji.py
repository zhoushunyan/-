...
def tabusearch(diedaitimes,cacu_time,tabu_length,origin_times,costf,printf):
    ...
    print(min(finall_cost))
    print(finall_road[index])
    printf(finall_road[index])
    for n in range(31):
    	x0=city[n-1][1][0]
    	y0=city[n-1][1][1]
    	plt.scatter(x0,y0,s=10,c='r')
    # x2=city[0][1][0]
    # y2=city[0][1][1]
    # x3=city[finall_road[index][0]][1][0]
    # y3=city[finall_road[index][0]][1][1]
    # plt.plot([x2,x3],[y2,y3],c='b')
    cityx=[]
    cityy=[]
    for m in range(29):
    	cityx.append(city[finall_road[index][m-1]][1][0])
    	cityy.append(city[finall_road[index][m-1]][1][1])
    	# x1=city[finall_road[index][m]][1][0]
    	# y1=city[finall_road[index][m]][1][1]
    plt.plot(cityx,cityy,c='b')
    # x2=city[finall_road[index][27]][1][0]
    # y2=city[finall_road[index][27]][1][1]
    # x3=city[0][1][0]
    # y3=city[0][1][1]
    # plt.plot([x2,x3],[y2,y3],c='b')
    plt.xlabel('x label')
...
tabusearch(diedaitimes=100,cacu_time=100,tabu_length=10,origin_times=200,costf=costroad,printf=printtravel)